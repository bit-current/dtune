...
# Prepare DataLoader
eval_dataloader = SubsetFineWebEdu2Loader(
    batch_size=4,
...
class ModelEvaluator:
    def __init__(self, model, data_loader, device="cuda" if torch.cuda.is_available() else "cpu"):
    ...
    def time_evaluation(self, num_runs=5):
        total_time = 0
        for i in range(num_runs):
            ...
            total_time += run_time
            print(f"Run {i+1}: Time: {run_time:.2f}s, Loss: {loss:.4f}, Perplexity: {perplexity:.2f}")
        
        avg_time = total_time / num_runs
        print(f"\nAverage evaluation time over {num_runs} runs: {avg_time:.2f} seconds")

# Create an instance of ModelEvaluator and run the timed evaluation
evaluator = ModelEvaluator(model, eval_dataloader)
evaluator.time_evaluation()