...
class BittensorNetwork:
    ...
    def get_validator_uids(
        cls, vpermit_tao_limit: int = 0.89
    ):
        """
    ...
    def get_validator_hotkeys(
        cls, vpermit_tao_limit: int = 1
    ):
        validator_hotkeys = []  # List to hold 'ip:port' strings
        for uid,hotkey in enumerate(cls.metagraph.hotkeys):
            if cls.metagraph.S[uid] >= vpermit_tao_limit:
                validator_hotkeys.append(hotkey)
